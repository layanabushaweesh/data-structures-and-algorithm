...
let string;
  arr.reduce((acc, str) => {
    string = acc.length > str.length ? acc : str
    return string;
  }, "a");
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  console.log(arr);
  return arr.map(str => str.split("").reduce((acc,char) => {
...
const onlyOddChars = (str) => {
  ...
  for(let i = 0; i < str.length; i++){
    let codeNo;
    codeNo = str.charCodeAt(i)
    if (codeNo % 2 !== 0 && codeNo < 97){
      string += str.charAt(i);
    }else if (codeNo % 2 == 0 && codeNo >= 97){
      string += str.charAt(i);
    }
  }
  return string;
